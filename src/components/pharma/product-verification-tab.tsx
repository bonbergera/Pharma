...
export function ProductVerificationTab() {
  ...
  const [productDetails, setProductDetails] = useState<{ name: string; manufacturer: string } | null>(null);
  ...
  const handleBarcodeScanSuccess = useCallback((decodedText: string) => {
    ...
    toast({
      ...
      description: "Serial number populated from barcode.",
    });
  ...
  const handlePackagingAnalysisComplete = useCallback((result: PackagingAnalysisResult) => {
    ...
    setIsAnalyzingPackaging(false);
    setShowPackagingAnalyzer(false); // Close dialog on completion
    toast({
  ...
  const handlePackagingAnalysisError = useCallback((errorMsg: string) => {
    setIsAnalyzingPackaging(false);
    // setShowPackagingAnalyzer(false); // Optionally keep dialog open on error
    toast({
  ...
  const handleSubmitVerification = async (event?: FormEvent) => {
    ...
    // Simulate API call for serial number verification
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    // Mock responses based on serial number for demonstration
    if (serialNumber === "VALID123") {
      setVerificationStatus("verified");
      setProductDetails({ name: "Amoxicillin 250mg", manufacturer: "PharmaCorp Inc." });
      toast({
        title: "Product Verified",
        description: `Serial Number ${serialNumber} is authentic.`,
        action: <ShieldCheck className="text-green-500" />,
      });
    } else if (serialNumber === "FAKE456") {
      setVerificationStatus("not_found"); // Or a specific "counterfeit_detected_by_serial" status
      toast({
        title: "Product Not Authentic",
        description: `Serial Number ${serialNumber} may not be authentic or is not registered.`,
        variant: "destructive",
        action: <ShieldX className="text-red-500" />,
      });
    } else {
  ...
  const renderVerificationResult = () => {
    if (verificationStatus === "idle" && !analysisResult) return null;
      ...
      <div className="mt-6 space-y-4">
        {productDetails && verificationStatus === "verified" && (
          <Alert variant="default" className="border-green-500 bg-green-50 dark:bg-green-900/30">
            ...
            <AlertDescription className="text-green-600 dark:text-green-400">
              ...
              <strong>Serial Number:</strong> {serialNumber} is registered and authentic.
            </AlertDescription>
          </Alert>
        )}
        {verificationStatus === "not_found" && (
          <Alert variant="destructive">
            ...
            <AlertTitle>Product Not Found or Suspect</AlertTitle>
            <AlertDescription>
              The serial number {serialNumber} was not found in our registry or is flagged as suspect. Please check the serial number and try again.
            </AlertDescription>
          </Alert>
        )}
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Enter the product serial number or use scanners to verify.
        </CardDescription>
      </CardHeader>
      <CardContent>
